...
import { ClientResponsibilityServices } from "./clientresponsibility.service";
...
const getSingleClientResponsibility = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
    const result =
      await ClientResponsibilityServices.getSingleClientResponsibilityFromDB(
        id
      );
    sendResponse(res, {
      statusCode: 200,
...
const updateClientResponsibility = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const { ...responsibilityData } = req.body;
    const result =
      await ClientResponsibilityServices.updateClientResponsibilityIntoDB(
        id,
        responsibilityData
      );
    sendResponse(res, {
